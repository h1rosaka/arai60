...
- CKY法
    ...
    - 「あるプログラムが、Pythonの言語の文法で書かれているかどうかの判定」も似たような方法で実施されている。
    - ![alt text](image.png)
...
- 再帰下降構文解析
    ...
    -  while(true)のブロックだけで、最後のサイズ確認がないと困るケース
        無関係な文字が含まれる
            例: "()abc"
                parens()は()を正しく処理し、consume('(')とconsume(')')が呼ばれる。
                その後、while (true)のループでは括弧に該当するものが見つからないためtrueを返して終了。
                しかし、"abc"が残っており未処理のまま。
                ここでi == s.size()がないと、これが誤って有効と判定される。
...
- https://github.com/hayashi-ay/leetcode/pull/16/files
    - 4th
    - ()の向きが揃ってる辞書なので見た目が良い。最後のreturnもわかりやすい
