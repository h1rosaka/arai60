...
- 計算量
    - 時間: ~~O(N)~~ O(NlogN)
        ...
        - total出す処理は、l1の長さとl2の長さ分だけ。その後答えを作るのにまた答えの桁数だけ。ただし、一回のループごとに**(累乗)の計算があり、これはCpythonの実装では繰り返し2乗法になっておりlogNかかる。
    - 空間: ~~O(1)~~　O(N)
        - ~~l1やl2が長くとも、処理中に覚えておかなければならないものの数は増えない。~~
        - l1やl2が長くとも、処理中に覚えておかなければならないものの数は増えないが、Pythonのintは2**(30*n)を超えるたびにメモリが4バイト追加で必要になる多倍長整数の形で実装されているため、Nが非常に大きい時は桁数に比例して必要メモリ数が増える。
- コメント
...
# STEP4 レビューFB反映
## 多倍長整数
配列を用いて整数を表現することで、大きな桁数の整数を扱えるようにするテクニック。
...
- Python(Cpython)での整数の実装
    - https://zenn.dev/yukinarit/articles/afb263bf68fff2
    - https://note.com/plasma_network/n/n6cddca4798c7
    - https://teratail.com/questions/m16piwih4m92i7
    - pythonでは2^30進数を用いて整数を表現している
    - 例としてもし10進数なら、234は、<4,3,2>と言った形で配列に格納
    - そのため2^30より小さい数を表すなら、配列のスペースは1マス分で良いが、それより1大きくなって2^30になると、<0,1>となって2マス必要になる。
    - その次に必要なマスが増えるのは、表したい数が(2^30)^2 = 2^60になった時
    - 配列の1マスが必要とするメモリのサイズは4バイト
    - 配列は、マス以外に参照カウントの情報や型の情報なども持っているため、余計に24バイト必要(整数は最低28バイト)
...
- x^nのような累乗は、普通に計算しようとすると、n回掛け算しなければならないので、計算量はO(n)
- しかしCpythonの実装では、「繰り返し２乗法」というテクニックを使って、計算量をO(logN)に抑えている。
    ```python
    def pow(x, n):
        ans = 1
        while n:
            if n % 2: #今見ているbit?2進数での桁?が0か1か確認
                ans *= x
            x *= x
            n >>= 1 #これはビット演算。右に一つシフトして、溢れた桁はなくす。(2で割るのと同じ効果がある。)
        return ans
    ```
- 解説
    - 3^10を考える。
    - 10=8+0+2+0なので、2進数で1010
    - 3^10　= (3^8) * (3^2) と変形できる
    - 1010を右端の0から順に見ていって、1の時だけ、必要な数をansにかけている
    - これは、nを2進数にしたときの桁数だけループするので、ループの回数はlogNかlogN+1になる。よって計算量はlogN
